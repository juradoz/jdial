...
class SessionHandlerImpl implements SessionHandler {
  ...
  SessionHandlerImpl() {
    ...
    configurations.put(null, configuration.configure());
  ...
  public synchronized Session getSession(String identifier) {
    if (!sessionFactories.containsKey(identifier)) {
      Configuration configuration = getConfiguration(identifier);
      sessionFactories.put(
          identifier,
          configuration.buildSessionFactory(new StandardServiceRegistryBuilder().applySettings(

...
public class DefaultTelefoneFilterTest {
  ...
  public void setUp() throws Exception {
    ...
    when(checker1.isUtil(tenant, t1)).thenReturn(true);
    when(checker1.isUtil(tenant, t2)).thenReturn(true);
    when(checker1.isUtil(tenant, t3)).thenReturn(true);
    when(checker2.isUtil(tenant, t1)).thenReturn(true);
    when(checker2.isUtil(tenant, t2)).thenReturn(true);
    when(checker2.isUtil(tenant, t3)).thenReturn(true);
    defaultTelefoneFilter = new DefaultTelefoneFilter(checkers);
  }

  @Test
  public void filterNaoDeveriaRemoverNenhum() throws Exception {
    List<Telefone> list = defaultTelefoneFilter.filter(tenant, asList(t1, t2, t3));
    assertThat(list.containsAll(asList(t1, t2, t3)), is(true));
  }

  @Test
  public void filterDeveriaFiltrart1() throws Exception {
    when(checker1.isUtil(tenant, t1)).thenReturn(false);
    List<Telefone> list = defaultTelefoneFilter.filter(tenant, asList(t1, t2, t3));
  ...
  public void filterDeveriaFiltrart2() throws Exception {
    when(checker1.isUtil(tenant, t2)).thenReturn(false);
    List<Telefone> list = defaultTelefoneFilter.filter(tenant, asList(t1, t2, t3));
  ...
  public void filterDeveriaFiltrart3() throws Exception {
    when(checker1.isUtil(tenant, t3)).thenReturn(false);
    List<Telefone> list = defaultTelefoneFilter.filter(tenant, asList(t1, t2, t3));
    assertThat(list.contains(t1), is(true));
    assertThat(list.contains(t2), is(true));
  ...
  public void filterDeveriaFiltrarTudo() throws Exception {
    when(checker1.isUtil(Mockito.eq(tenant), Mockito.any(Telefone.class))).thenReturn(false);
    List<Telefone> list = defaultTelefoneFilter.filter(tenant, asList(t1, t2, t3));

...
class DefaultPredictiveCallListener implements CallControlConnectionListener {
  ...
  private void logaEvento(Object event) {
    logger.debug(getMethodName() + " "
        + ToStringBuilder.reflectionToString(event, ToStringStyle.SHORT_PREFIX_STYLE).toString());
  }
  ...
  public void connectionAlerting(CallControlConnectionEvent event) {
    ...
    logger.debug(
        ...
        new Object[] {
            event.getCallingAddress() == null ? "null" : event.getCallingAddress().getName(),
            event.getCalledAddress() == null ? "null" : event.getCalledAddress().getName(),
            event.getCallingTerminal() == null ? "null" : event.getCallingTerminal().getName(),
    ...
    for (Connection connection : event.getCall().getConnections()) {
      ...
      if (connection.getTerminalConnections() != null)
        for (TerminalConnection tc : connection.getTerminalConnections()) {
          logger.debug("terminalConnection {}", tc.getTerminal().getName());
          if (((LucentTerminal) tc.getTerminal()).getAgents() != null) {
            for (Agent agent : ((LucentTerminal) tc.getTerminal()).getAgents()) {
              logger.debug("agent {}", agent.getAgentID());
              try {
                predictiveListener.chamadaNoAgente(extraiCallId(event), agent.getAgentID());
              } catch (Exception e) {
                logger.error(e.getMessage(), e);
              }
            }
